...
export function mapLoginEndpoint(
  ...
  app.post("/login", zValidator("form", loginRequestSchema), async (c) => {
    ...
    const isValidPassword = await Bun.password.verify(password, user.passwordHash, "bcrypt");
    if (!isValidPassword) {
      return c.text("Invalid username or password", 401);
    }
  ...
  return [
    {
      ...
      username: "admin",
      passwordHash: Bun.password.hashSync("password1234", {algorithm: "bcrypt", cost: 10}),
      role: "Administrator",
    },
    {
